...
%% Solving the governing equation using line by line solver by making tdma structure at half time step as indicated in the crank nicholson scheme
...
y_positions = (0:n-1) * delta_x * 1000;
x_positions = (0:n-1) * delta_x * 1000; 
...
legend({'0.00', '148.5 ',  '297.0'   ,'445.5' ,'594.0','742.5'}, 'Location', 'best')
hold off;
...
ylabel('Temperature (K)');
title('Temperature Variation along Horizontal Centerline (y = 40 mm)');
legend({'0.00', '148.5 ',  '297.0'   ,'445.5' ,'594.0','742.5'}, 'Location', 'best')
grid on;
hold off;
...
colorbar;
title('Temperature Field - 81 x 81 Using Crank-Nicolson Scheme');
xlabel('X');
ylabel('Y');
axis equal tight;
...
function [T, converged_time, vertical_data, horizontal_data, captured_times] = line_by_line_solver(T, n, delta_x, delta_t, h, lambda, T_inf, tolerance, thermal_diffusivity, target_times, vertical_data, horizontal_data, captured_times, current_index, time_instance)
    ...
    for k = 1:max_time_steps
        ...
        % Row sweep (implicit in x-direction)
        for i = 2:n-1
            n_unknowns = n - 2;
            ...
            for j_idx = 1:n_unknowns
                ...
                % Main diagonal - Crank-Nicolson
                A(j_idx, j_idx) = (1 + thermal_diffusivity * delta_t / (delta_x^2));
                
                % Off-diagonals in x-direction
                if j_idx > 1
                    A(j_idx, j_idx-1) = -(thermal_diffusivity * delta_t / (delta_x^2))/2;
                else
                    % Left boundary contribution
                    A(j_idx, j_idx) = A(j_idx, j_idx) - ((thermal_diffusivity * delta_t / (delta_x^2))/2)*(lambda/(lambda + h*delta_x));
                end
                ...
                % rhs matrix
                b(j_idx) = T_old(i,j) * (1 - (thermal_diffusivity * delta_t / (delta_x^2))) + ((thermal_diffusivity * delta_t / (delta_x^2))/2) * (T_old(i,j-1) + T_old(i,j+1)) + ((thermal_diffusivity * delta_t / (delta_x^2))/2) * (T_old(i-1,j) - 2*T_old(i,j) + T_old(i+1,j)) + ((thermal_diffusivity * delta_t / (delta_x^2))/2) * (T(i-1,j) - 2*T(i,j) + T(i+1,j));
                
                % Boundaries
                if j_idx == 1
                    b(j_idx) = b(j_idx) + ((thermal_diffusivity * delta_t / (delta_x^2))/2) * ((h*delta_x*T_inf)/(lambda + h*delta_x));
                elseif j_idx == n_unknowns
                    b(j_idx) = b(j_idx) + ((thermal_diffusivity * delta_t / (delta_x^2))/2) * T(i, n);
                end
            end
